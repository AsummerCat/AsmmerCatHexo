...
# 注意使用API有两种方式
1. 原生的RestHighLevelClient
2. JPA带的ElasticsearchRepository
...
# 使用 JPA带的ElasticsearchRepository
需要创建一个实体
```
@Document(indexName = "user") //标记document的index名称
@Data
public class Person {
...
```
## 更新数据
<font color="red">注意: 如果document不存在则不更新</font>
```
/**
	 * 更新数据
...
```
 public void updateData() throws IOException {
        ...
        UpdateRequest updateRequest = new UpdateRequest("user",  "1")
                ...
                .upsert(indexRequest);
        //执行更新语法
        restHighLevelClient.update(updateRequest, RequestOptions.DEFAULT).getGetResult();

    }

```

## 查询:一次性获取数据 不存在则为null
等同于 mget 
```
...
## 基于bulk批量插入
<font color="red">增删改的操作 可以在同一个bulk下提交</font>
```
 /**
     * 批处理上传
     * @throws IOException
